...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    if config.GENERATE_MASKS:
        image, window, scale, padding, crop = utils.resize_image_old(
            image,
        ...
        image, window, scale, padding, crop, bbox = utils.resize_image(
            image,
            min_dim=config.IMAGE_MIN_DIM,
            min_scale=config.IMAGE_MIN_SCALE,
            max_dim=config.IMAGE_MAX_DIM,
    ...
    if augmentation=='balloon':
        logging.warning("'augmentation' balloon is deprecated. Use 'augment' instead.")
    ...
    if augmentation and augmentation!='balloon':
        import imgaug
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, config, gt_masks):
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augment=True, augmentation=None,
                        ...
                        if self.detection_targets:
                            rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask =\
                                build_detection_targets(
                                    rpn_rois, gt_class_ids, gt_boxes, self.config, gt_masks)
                    else:
                        if self.detection_targets:
                            rois, mrcnn_class_ids, mrcnn_bbox =\
                                build_detection_targets(
                                    rpn_rois, gt_class_ids, gt_boxes, self.config, gt_masks=None)
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        ] #chng: orginally save_weights_only=True
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            if self.config.GENERATE_MASKS:
                molded_image, window, scale, padding, crop = utils.resize_image_old(
                    image,
    ...
    def detect(self, images, verbose=0):
        ...
        #print('self.mold_inputs(images) time is {}'.format(time.time() - t1))
        ...
        if self.config.GENERATE_MASKS:
            ...
            detections, _, _, _, _, _ =\
                self.keras_model.predict([molded_images, image_metas, anchors], verbose=0)
            #print('self.keras_model.predict([molded_images, image_metas, anchors], verbose=0) time is {}'.format(time.time() - t4))
            # Process detections
            results = []
            for i, image in enumerate(images):
